...
export function renderTitle(x, y, node, output, position, zIndexRoot) {
    ...
    const line = new Array(contentWidth).fill(fillChar === " "
        ? colorize(fillChar, node.style.backgroundColor, "background")
        : fillChar);
    // prevent overflow chars
    const getEnd = (len) => Math.min(len, contentWidth);
...
function styleTitle(text, title) {
    if (!title)
        return text;
    return styleText(title)(text);
}
function getPosition(sx, sy, dxs, dxe, displaceHeight, hasPadding) {
    if (displaceHeight) {
        sy += displaceHeight - 1;
    }
    if (hasPadding) {
...
function getBox(node, colorized) {
    const immutableBox = typeof node.style.borderStyle === "string"
    ...
    const colorFactory = (colorStr) => (str) => {
...
// 	return [dx, dy];
// }
//# sourceMappingURL=renderTitleToOutput.js.map