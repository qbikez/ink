...
export class ScrollAPI {
    ...
    centerScroll;
    fallthrough;
    constructor({ state, setState, LENGTH, WINDOW_SIZE, opts, prevBounds }) {
        this.state = state;
    ...
    handle = (nextIdx = this.state.idx) => {
        nextIdx = Math.floor(nextIdx);
        if (nextIdx >= this.LENGTH || nextIdx < 0)
            nextIdx = this.state.idx;
        try {
    ...
    nextItem = () => {
        ...
        if (this.state.idx >= this.LENGTH - 1)
            return;
    ...
    getNormalScrollChanges = (nextIdx = this.state.idx) => {
        nextIdx = Math.floor(nextIdx);
    ...
    getCenterScrollChanges = (nextIdx = this.state.idx) => {
        nextIdx = Math.floor(nextIdx);
        const LENGTH = this.LENGTH;
        const WINDOW_SIZE = this.WINDOW_SIZE;
...
//# sourceMappingURL=ScrollAPI.js.map