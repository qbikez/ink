...
export const WindowAttributes = { viewState: "viewState" };
export function Window({ ...props }) {
    ...
    function getItem(node, idx, listeners = []) {
        const key = node.key;
        ...
        return (React.createElement(Unit, { key: key, type: props.type, node: node, listeners: listeners, isHidden: isHidden, 
            // context
            isShallowFocus: isShallowFocus, isDeepFocus: isDeepFocus, index: idx, items: props.viewState._items, setItems: props.viewState._setItems, control: props.viewState._control, stretch: props.viewState._unitSize === "stretch" }));
    }
    function handleMap(item, idx) {
        const listeners = [];
        const isFocus = idx === props.viewState._idx;
        const onUnit = (event, handler) => {
            if (!isFocus || !THIS_WINDOW_FOCUS)
                return;
            if (!ModalStack.isActiveModalLevel(THIS_WINDOW_LEVEL))
                return;
        ...
        const node = isRenderable(item)
            ? item
            : item(isFocus, onUnit);
        return getItem(node, idx, listeners);
    }
    function getSlicedItems() {
        ...
        const sliceEnd = Math.max(props.viewState._start + batchSize, props.viewState._end);
        const slicedItems = items.slice(sliceStart, sliceEnd);
        const toRender = [];
        for (let i = start; i < end; ++i) {
            if (slicedItems[i] === undefined)
                continue;
    ...
    return (React.createElement(WindowContext.Provider, { value: { isFocus: THIS_WINDOW_FOCUS } },
        React.createElement("ink-window", { style: { ...props }, viewState: props.viewState }, props.batchMap ? getSlicedItems() : generatedItems)));
}
