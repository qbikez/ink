...
import { WindowAttributes } from "./Window.js";
import colorize from "../colorize.js";
import chalk from "chalk";
export function renderWindowToOutput(x, y, node, output) {
    const viewState = node.attributes[WindowAttributes.viewState];
...
export function renderScrollbar(x, y, node, output) {
...
function getScrollbarString(node, viewState, scrollbar) {
    const height = node.yogaNode.getComputedHeight();
    const width = node.yogaNode.getComputedWidth();
    let colChar = "";
    let rowChar = "";
    ...
    if (node.style.flexDirection === "column") {
        const barLength = Math.max(0, Math.ceil(height * (Math.min(_itemsLen, _winSize) / _itemsLen)));
        ...
        const barLength = Math.max(0, Math.ceil(width * (Math.min(_itemsLen, _winSize) / _itemsLen)));
...
function colorizeBar(bar, scrollbar) {
...
//# sourceMappingURL=renderWindowToOutput.js.map