...
const renderNodeToOutput = (node, output, options, zIndexes = []) => {
    ...
    const InternalStyles = node.attributes["internalStyles"];
    ...
    const hasZIndex = typeof node.style.zIndex === "number" &&
        node.style.zIndex > 0 &&
    ...
    if (node.nodeName === "ink-box" && hasZIndex) {
        // prettier-ignore
        const nodeZIndex = node.style.zIndex + (options.rootZIndex ?? 0);
        // prettier-ignore
        const cb = () => {
