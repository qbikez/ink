...
export const ID_NAV_EVENTS = {
    up: (ID) => `${ID}-${BASE_EVENTS.up}`,
    down: (ID) => `${ID}-${BASE_EVENTS.down}`,
    left: (ID) => `${ID}-${BASE_EVENTS.left}`,
    right: (ID) => `${ID}-${BASE_EVENTS.right}`,
    next: (ID) => `${ID}-${BASE_EVENTS.next}`,
    prev: (ID) => `${ID}-${BASE_EVENTS.prev}`,
};
export const VI_KEYMAP = (ID) => {
    return {
