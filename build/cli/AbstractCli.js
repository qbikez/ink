...
import { Key, Text, TextInput, useKeymap, Box, useInput, } from "../index.js";
...
export function AbstractCli(props) {
    ...
    return (React.createElement(CliView, { ...cliViewProps, value: value, onChange: onChange, setValue: setValue, insert: insert, enterInsert: enterInsert, textStyles: textStyle, autoEnter: autoEnter, onUpArrow: onUpArrow, onDownArrow: onDownArrow, promptStyles: props.promptStyles, actionPrompt: props.actionPrompt }));
}
function CliView({ commands, enterKeymap = { input: ":" }, exitKeymap = [{ key: "return" }, { key: "esc" }], prompt = ":", persistPrompt = false, promptStyles, onChange, setValue, value, insert, textStyles, hideModal, onDownArrow, onUpArrow, autoEnter, enterInsert, actionPrompt = ["", () => { }], }) {
    const prefixValue = () => {
    ...
    return (React.createElement(Box, { width: "100", flexDirection: "row", backgroundColor: "inherit" },
        React.createElement(Box, { height: 1, width: prefixValue().length, flexShrink: 0 },
            React.createElement(Text, { wrap: "truncate-end" }, prefixValue())),
        showAP && (React.createElement(Box, { height: 1, width: actionPromptText.length ?? 0, flexShrink: 0 },
            React.createElement(Text, { wrap: "truncate-end", styles: promptStyles }, actionPromptText))),
        React.createElement(Box, { height: 1, width: "100", flexShrink: 2 },
            React.createElement(TextInput, { textStyle: textStyles, onChange: onChange, enterKeymap: enterKeymap, exitKeymap: exitKeymap, autoEnter: autoEnter, onDownArrow: onDownArrow, onUpArrow: onUpArrow, 
                ...
                onExit: (value, stdin) => {
                    ...
                    promise
                        ...
                        .catch((error) => {
                        const sanitized = sanitizeResult(error);
                        setImmediate(() => {
 ...
 * */
async function handleInput(commands, cliInput) {
...
function toRawInput(cliInput, command) {
    return cliInput.replace(command, "").trimStart().trimEnd();
}
function toSanitizedArray(parsedInput) {
    return parsedInput
        .trimStart()
        .trimEnd()
        .split(" ")
...
function sanitizeResult(result) {
    const replace = (result) => result.replace(/\t|\n/g, " ");
...
//# sourceMappingURL=AbstractCli.js.map